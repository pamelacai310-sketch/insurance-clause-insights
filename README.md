...
- 对每个条款标签做同类频率校正：样本命中率低于 `25%` 才标为“真罕见条款”，高频功能按通用功能处理
...
- 保险期间、缴费期间、等待期等关键字段
- 3 条最独特的特色及对应证据句
- `feature_audit`、`common_features`、`relative_features`、`rare_features`，用于区分通用功能、相对少见功能和真罕见条款

## 分析说明

- “独特特色”目前使用规则抽取 + 文本相似度对比完成
- “罕见条款”使用频率校正口径：同类样本命中率 `<25%` 才算真罕见；`>=75%` 的高频标签只作为基础配置，不作为独有卖点
