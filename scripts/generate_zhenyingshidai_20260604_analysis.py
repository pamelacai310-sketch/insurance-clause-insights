...
from analysis_governance import (  # noqa: E402
    AdvantageClaim,
...
def write_actuarial_report(products: list[dict[str, Any]]) -> None:
    ...
    lines.extend(
        render_advantage_validation_table(
            [
                AdvantageClaim(
                    "终身保障",
                    f"{target_audit.get('终身保障', {}).get('peer_count', 0)}/21",
                    target_audit.get("终身保障", {}).get("peer_count", 0),
                    21,
                    True,
                    "样本标配，只能作为准入条件。",
                ),
                AdvantageClaim(
                    "红利用于增额",
                    f"{target_audit.get('红利用于增额', {}).get('peer_count', 0)}/21",
                    target_audit.get("红利用于增额", {}).get("peer_count", 0),
                    21,
                    False,
                    "分红型终身寿险常见机制，且红利不保证；需比较红利方式和演示结果。",
                ),
                AdvantageClaim(
                    "可保单贷款",
                    f"{target_audit.get('可保单贷款', {}).get('peer_count', 0)}/21",
                    target_audit.get("可保单贷款", {}).get("peer_count", 0),
                    21,
                    True,
                    "样本标配，不构成差异化。",
                ),
                AdvantageClaim(
                    "第二投保人设计",
                    f"{target_audit.get('第二投保人设计', {}).get('peer_count', 0)}/21",
                    target_audit.get("第二投保人设计", {}).get("peer_count", 0),
                    21,
                    True,
                    "相对少见，但汇丰同系列和友邦部分产品也有。",
                ),
                AdvantageClaim(
                    "双意外额外给付",
                    f"航空{target_audit.get('含航空意外额外身故给付', {}).get('peer_count', 0)}/21，重大自然灾害{target_audit.get('含重大自然灾害意外身故给付', {}).get('peer_count', 0)}/21",
                    max(
                        target_audit.get("含航空意外额外身故给付", {}).get("peer_count", 0),
                        target_audit.get("含重大自然灾害意外身故给付", {}).get("peer_count", 0),
                    ),
                    21,
                    True,
                    "属相对稀缺的保障杠杆增强点。",
    ...
    lines.extend([""])
    lines.extend(render_external_risk_audit("分红型终身寿险", "汇丰人寿"))
    lines.extend([""])
    lines.extend(render_target_implications("汇丰臻盈世代"))
    lines.extend(
        [
            "",
            "## 目标产品公开演示表IRR",
...
def write_persona_report(products: list[dict[str, Any]]) -> None:
    ...
    lines = [
        ...
        "## 外部风险下的适配边界",
        "",
        "- 适合接受“保证利益 + 非保证分红”分层的人群：客户需要明白保证现金价值、保证身故/全残责任和红利演示不是同一层级，不能把情景2利益当作刚性收益。",
        "- 适合重视长期配置和传承治理的人群：第二投保人、受益人安排、终身责任和交清增额机制更偏家庭治理工具，而不是短期收益工具。",
        "- 适合愿意持续跟踪保险公司公开披露的人群：分红型产品长期表现受保险公司资产负债匹配、分红账户经营、偿付能力和再保险安排影响，建议至少年度复核偿付能力报告、风险综合评级、分红实现情况和资产配置说明。",
        "- 不适合把分红演示收益视为刚兑的人群：低利率、长久期负债、再保险资本优化、衍生品保证金压力和私募资产流动性风险都可能影响行业分红稳定性，合同只保证明确写入条款和现金价值表的利益。",
        "- 不适合高度关注短期现金价值回收的人群：目标产品第1年和第6年保证退保给付明显低于累计保费，如果家庭现金流可能频繁中断，应优先配置高流动性资产。",
        "",
        "## 不适合人群",
        "",
        ...
        "- 不愿阅读或持续关注保险公司偿付能力、资产配置和分红实现披露，却希望长期分红高度确定的客户。",
    ]
